...
export class HTMLPage extends Page {
    ...
    public newTemporaryCopy(): Page {
        // In Portrait mode, we force all pages to be SOFT so they curl smoothly.
        if (this.render.getOrientation() !== Orientation.PORTRAIT && this.nowDrawingDensity === PageDensity.HARD) {
            return this;
        }
        ...
        if (this.temporaryCopy === null) {
            ...
            this.temporaryCopy = new HTMLPage(
                this.render,
                this.copiedElement,
                PageDensity.SOFT // Force temporary copy to be soft
            );
        }
    ...
    public draw(tempDensity?: PageDensity): void {
        ...
        // Force SOFT rendering in PORTRAIT mode for a consistent peel effect
